...
| `.tafel` + `.brett` + `.flaps` | Abflugtafel | papier-tief, Buchstaben-Kacheln (JS baut aus data-zeile). Desktop: Grid 4 Spalten. Mobil (<720px): kein Querscroll mehr — `.brett` wird Flex-Stapel, jede `.flapzeile` eine Boarding-Karte (Ziel gross / Detail / Preis + Status-Chip). Trailing-Fuellkacheln (`b.pad`, von JS markiert) mobil ausgeblendet, interne Wortabstaende bleiben. Scroll-Indikator .tafel-scroll nur Desktop |
...
| `.marquee` + `.mini-tag` / `.koffer-ic` | Gepaeckband-Ticker | Endlos-Loop (2 Gruppen, JS-Dauer ~13px/s), Gepaeck-Anhaenger leicht rotiert |
