...
| `.shot-window` / `.live-stage` | Live-Browserfenster | ink-Rahmen + Offset-Schatten, Geraete-Buttons aria-pressed, .live-cue Hinweis-Pille; iframe bekommt echte Geraetebreite (mac 1440 / tablet 768 / iphone 375) + wird skaliert; iPhone zusaetzlich feste Viewporthoehe (HOEHEN.iphone=740) -> echte dvh im iframe, proportional eingepasst + zentriert; iframe-Reload beim Kreuzen der 860px-Grenze, damit Routenwerk frisch in Desktop- bzw. Mobile-Variante bootet |
...
| `.beleg-mobil` (+ `.bm-hero/-phone/-badge/-cap/-proof/-pr`) | Belege MOBIL (angebot, NUR <901px) | Handy-first (Marvins Wahl aus 3 Mobil-Varianten `_varianten-mobile.html`): grosses freigestelltes iPhone als Held, PageSpeed-98/100-Marke an der oberen rechten Ecke verankert, restliche 3 Werte (0/AA/100%) als kompakte Liste `.bm-pr`. KEIN Desktop-Bild auf Mobil (spart Laenge, Marvins Wunsch). Ab 901px `#belege .beleg-mobil{display:none}`, Desktop zeigt `.collage`. Loeste den ueberlappenden `.showcase` ab (CSS entfernt). |
| `.collage` (+ `-deko/-browser/-bar/-phone/-tags`) | Diagonale Collage (angebot, NUR DESKTOP >=901px) | Marvins Wahl aus 5 Varianten (`_varianten-desktop.html`). Gekipptes Browserfenster (rotate -1.4deg, Offset-Schatten) rechts, freigestelltes iPhone (rotate +2deg) unten links davor, Riesen-Outline-Ziffer `98` (clamp 240-360px, text-stroke --signal, opacity .5) als Grafik dahinter, 4 Mono-Beleg-Chips (ink-Grund, Wert in --signal-tint) links gestapelt. Ersetzt showcase+facts auf Desktop (alle 4 Belegwerte stecken in den Chips). Bewusster leichter Bruch mit der strengen Orthogonale (Marvin abgesegnet). Alles `#belege`-gescopt (.showcase/.facts existieren auch auf projekt-routenwerk). |
