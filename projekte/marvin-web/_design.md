...
| `.showcase` (+ `-browser/-bar/-dots/-url/-phone/-cap`) | Geraete-Showcase (angebot, NUR MOBIL <901px) | Browserfenster mit Desktop-Screenshot `img/demo-routenwerk.jpg` + freigestelltes iPhone unten rechts. Ab 901px per `#belege .showcase{display:none}` ausgeblendet, Desktop zeigt stattdessen `.collage`. |
