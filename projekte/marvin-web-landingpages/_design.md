...
| `.karte .antwort` | Ehrliche-Antwort-Zeile (Selbsttest, nur eigentum) | Beleg-Zeile unter der Frage: dashed Top-Border, Mono 13.5, Label muted + Wert als Block in --signal-deep. Im 3-Spalten-Layout: Karten sind flex-column, .antwort margin-top:auto + min-height 56px (2 Zeilen), damit Trennlinie und Antwort ueber alle drei Karten FLUCHTEN, egal wie lang die Frage ist; <=620px (einspaltig) min-height 0 (2026-07-20) |
...
| `.showcase` | Geraete-Showcase | Browserrahmen (.showcase-bar mit Dots + URL) + Desktop-Screenshot, davor .showcase-phone = freigestelltes iPhone-Foto-Mockup (Marvin 2026-07-18, Photoroom-Freistellung, scharfer routenwerk-Screenshot drin). Ausgeliefert als `<picture>`: transparentes AVIF (49KB, Hauptbild) + PNG-Fallback (376KB, nur alte Browser). Seit 2026-07-20 v2 mit Marvins Button-Fix im Screenshot; Quelle _src/mockup/routenwerk-iphone-photoroom-v2-button-fix.png war nur 1000x750 (Handy 277px breit) und wurde 1,58x hochskaliert, damit Canvas/Framing identisch 500x973 bleiben (Handy-Bbox x31/y42, wie v1) - bei sichtbarer Unschaerfe Photoroom-Export in hoeherer Aufloesung nachfordern. CSS: 184px, `filter:drop-shadow` (weich, folgt der Alpha-Silhouette) statt CD-Hard-Shadow; KEINE border/radius/background mehr - das Foto-Geraet bringt den Rahmen selbst mit. Assets: img/showcase-phone-mockup.{avif,png}, Originale in _src/mockup/. STATISCH, bewusst kein iframe. WICHTIG: unter 860px .showcase-cap padding-right (176/132), sonst haengt das Handy ueber der Bildunterschrift. Unter 560px Browser-Leiste eine Stufe kleiner (bar-padding 7/10, Dots 8px, URL 11px bei padding 4/9; Marvin 2026-07-20: Leiste war auf Mobile zu dominant, Screenshot soll dominieren). HINWEIS: Browser-Pane rendert AVIF nicht (Headless), Verify nur ueber den PNG-Fallback moeglich; AVIF-Qualitaet per avif->png-Rueckwandlung geprueft |
