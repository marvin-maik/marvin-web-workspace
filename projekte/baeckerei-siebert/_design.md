...
| `.sortiment-split` | Sortiment-Teaser (index) Bild+Tafel | Bild `img/auslage-regal.jpg` (Wand aus Brot, Hochformat-Datei) in `.anker maske bild`. RESPONSIVE Ratio via CSS (nicht inline): **mobil & tablet <1000px = 16:9-Band volle Breite, gestapelt** (`object-position:50% 50%`); **ab 1000px = Grid `520px 1fr`** Bild links `aspect-ratio:3/4` (`object-position:50% 60%`) + Preistafel/CTA rechts (`align-items:center`). Loest das hohe Foto: kompakt auf klein, Hochformat-Split auf gross |
